-- Sözleşme devri (subentro) artık sözleşme tipinden ayrı bir bayrak: asıl sözleşme tipi (öğrenci / geçici / uzun süreli) ayrı tutulur.
-- Eski kayıtlarda tip olarak saklanan 'Subentro (Resmi Sözleşme Devri)' değeri korunur ve bayrak true yapılır; sihirbaz
-- bu ilanlar düzenlenirken gerçek tipin seçilmesini ister.
alter table public.listings add column is_subentro boolean not null default false;
update public.listings set is_subentro = true where contract_type = 'Subentro (Resmi Sözleşme Devri)';

-- Radar: yalnızca devir ilanları.
alter table public.listing_radars add column only_subentro boolean not null default false;

-- Bildirim fonksiyonu: devir koşulu eklendi (öncekiyle aynı, tek satır fark).
...
begin
  ...
  -- Kısa dönem (en fazla 6 ay) / uzun dönem: müsaitlik aralığı en yakın aya yuvarlanır (utils/rentalTerm.ts ile aynı).
  if v_start is not null and v_end is not null and v_end > v_start
     and greatest(1, round((v_end - v_start)::numeric / 30.4375)) <= 6 then
    v_term := 'short';
  end if;
  ...
  begin
    with matched as (
      ...
      where r.active
        ...
        and (r.rental_term is null or r.rental_term = v_term)
        and (not r.only_subentro or new.is_subentro or new.contract_type = 'Subentro (Resmi Sözleşme Devri)')
        ...
        -- Seçilen günlerin TAMAMINDA müsait olan ilanlar (istemci filtresiyle aynı): ilk günden önce başlamış, son günden sonra bitiyor.
        and (r.start_from is null or v_start is null or v_start <= r.start_from)
        and (r.start_from is null or v_end is null or v_end >= coalesce(r.start_to, r.start_from))
