-- 1) 30 gün ve daha kısa kalışlar yayınlanamaz (turistik kiralama / CIN kapsamına girer): süre en az 31 gün olmalı.
--    Alanlar 0013 ile zorunlu ve biçimi doğrulanmış olduğundan tarih dönüşümü güvenlidir.
alter table public.listings
  add constraint listings_min_stay_days check (
    (contract_end_iso::date - contract_start_iso::date) >= 31
  );

-- 2) Radar: süre sekmesi (boş = farketmez; short = en fazla 6 ay, long = 6 aydan uzun).
alter table public.listing_radars
  add column rental_term text check (rental_term is null or rental_term in ('long', 'short'));

-- 3) Bildirim fonksiyonu: sekme eşleşmesi ve "seçilen günlerin tamamında müsait" tarih mantığı.
